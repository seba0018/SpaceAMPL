...
#Generic
    ...
    param epsilon default    0.5;                  # Tunes the aggressivity of the optimal solution
...
#Quadrotor params
    ...
    param k_p      default  1.41193310e-09;
    param k_pv     default  -7.97101848e-03;
    param k_q      default  1.21601884e-09;
    param k_qv     default  1.29263739e-02;
    
    param k_r1     default 2.57035545e-06;
    param k_r2     default 4.10923364e-07;
    param k_rr     default 8.12932607e-04;
    
    param k_x      default 1.07933887e-05;
    param k_y      default 9.65250793e-06;
    
    param k_omega  default 4.36301076e-08;
    param k_z      default 2.78628990e-05;
    param k_h      default 6.25501332e-02;
    
    param Mx_ext default  0;
    param My_ext default  0;
    param Mz_ext default  0;
    
    param omega_max default    11300;   # [rpm] Max min per rotor
    param omega_min default    3000;    # [rpm] Min rpm per rotor
    
    param omega_hover default  sqrt(g/(k_omega*4));
    param u_hover default      (omega_hover-omega_min)/(omega_max-omega_min);
    
    param tau default          0.03;                 # [s] First order delay constant for controls
...
# First waypoint    
    param wp_threshold default  0.05;      # [m]
    var max_offset_wp <= wp_threshold^2;  # [m] the power of 2 allows to skip a computation when computing the distance to WP

    param fov_angle_threshold default  45*pi/180;      # [rad]
    var fov_angle <= fov_angle_threshold;  # [rad] half the fov angle for heading constraint at WP1 such that you see WP2

    param wp1_x default        1.0;        # [m] First waypoint dx
    param wp1_y default        1.0;        # [m] First waypoint dy
    param wp1_z default        1.0;        # [m] First waypoint dz
    param wp1_psi default      0.0;        # [rad] First waypoint yaw angle
...
#Constraints------------------------------------------
    ...
    #Final
    subject to FinalPositionx :  dx[n] = 0;
    subject to FinalPositiony :  dy[n] = 0;
    subject to FinalPositionz :  dz[n] = 0;
    
    #World coordinates
    subject to FinalVelocityx : vx[n]*cos(psi[n])*cos(theta[n]) + 
                                vy[n]*(sin(phi[n])*sin(theta[n])*cos(psi[n]) - sin(psi[n])*cos(phi[n])) + 
                                vz[n]*(sin(phi[n])*sin(psi[n]) + sin(theta[n])*cos(phi[n])*cos(psi[n])) 
                                = v_final*cos(psin);
                                
    subject to FinalVelocityy : vx[n]*sin(psi[n])*cos(theta[n]) + 
                                vy[n]*(sin(phi[n])*sin(psi[n])*sin(theta[n]) + cos(phi[n])*cos(psi[n])) + 
                                vz[n]*(-sin(phi[n])*cos(psi[n]) + sin(psi[n])*sin(theta[n])*cos(phi[n])) 
                                = v_final*sin(psin);
                                
    subject to FinalVelocityz : -vx[n]*sin(theta[n]) + 
                                vy[n]*sin(phi[n])*cos(theta[n]) + 
                                vz[n]*cos(phi[n])*cos(theta[n]) 
                                = 0;
    ...
    subject to FinalRollRate  : p[n]   = 0;
    subject to FinalPitchRate : q[n]   = 0;
    subject to FinalYawRate   : r[n]   = 0;
...
##### Constraint for first Waypoint
    
    var x_world{i in I} = -dx[i]*cos(psi[i])*cos(theta[i]) + 
                                     dy[i]*(-sin(phi[i])*sin(theta[i])*cos(psi[i]) + sin(psi[i])*cos(phi[i])) + 
                                     dz[i]*(-sin(phi[i])*sin(psi[i]) - sin(theta[i])*cos(phi[i])*cos(psi[i]));
    
    var y_world{i in I} = -dx[i]*sin(psi[i])*cos(theta[i]) + 
                                     dy[i]*(-sin(phi[i])*sin(psi[i])*sin(theta[i]) - cos(phi[i])*cos(psi[i])) + 
                                     dz[i]*(sin(phi[i])*cos(psi[i]) - sin(psi[i])*sin(theta[i])*cos(phi[i]));
    
    var z_world{i in I} =  dx[i]*sin(theta[i]) - 
                                     dy[i]*sin(phi[i])*cos(theta[i]) -
                                     dz[i]*cos(phi[i])*cos(theta[i]);
    
    #var distance_to_wp1{i in I} = (x_world[i]-wp1_x)^2 + 
    #                              (y_world[i]-wp1_y)^2 + 
     #                             (z_world[i]-wp1_z)^2;
                                  
    #subject to Distance_constraint_wp1 : min{i in I} distance_to_wp1[i] = max_offset_wp;

                                  
    var distance_and_yaw_diff_to_wp1{i in I} = (x_world[i]-wp1_x)^2 + 
                                  (y_world[i]-wp1_y)^2 + 
                                  (z_world[i]-wp1_z)^2 +
                                  (psi[i] - wp1_psi)^2;
    
    subject to Distance_and_yaw_constraint_wp1 : min{i in I} distance_and_yaw_diff_to_wp1[i] = max_offset_wp;


    #var heading_x_world{i in I} = -1*cos(psi[i])*cos(theta[i]);
    
    #var heading_y_world{i in I} = -1*sin(psi[i])*cos(theta[i]);
    
    #var heading_z_world{i in I} =  1*sin(theta[i]);

    #var angle_heading_wp2{i in I} = acos( ( (((dx[i])^2+
     #                                       (dy[i])^2+
      #                                      (dz[i])^2)^0.5)*
       #                                     ((1)^2+
        #                                    (0)^2+
         #                                   (0)^2)^0.5 )
          #                                 /( (dx[i])*(1)+
           #                                   (dy[i])*(0)+
            #                                  (dz[i])*(0) ) );
    #subject to wp2_in_fov : min{i in I} angle_heading_wp2[i] = fov_angle;
...
#Solver Options-----------------------------------------------
    ...
    option substout 0;
    option show_stats 1;
    options snopt_options "outlev=2 Major_iterations=1500 Superbasics=1500";
#-------------------------------------------------------------
